...
export default function CategoryManagementPage() {
  ...
  const fetchCategories = async () => {
    try {
      ...
      if (!token) return;
      const res = await fetch(`${baseUrl}/api/user/categories`, {
    ...
    try {
      ...
      if (editingId) {
        ...
        if (!res.ok) {
          ...
          throw new Error(txt || 'Failed to update category');
        }
        ...
        if (!res.ok) {
          const txt = await res.text();
          throw new Error(txt || 'Failed to add category');
        }
                        ...
                        <div className="flex gap-2">
                          ...
                          <div className="relative">
                            <button
                              onClick={() =>
                                setDeleteConfirmId(
                                  deleteConfirmId === category.id ? null : category.id
                                )
                              }
