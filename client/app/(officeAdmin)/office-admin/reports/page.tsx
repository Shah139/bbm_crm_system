...
export default function ReportsPage() {
  ...
  useEffect(() => {
    const loadShowrooms = async () => {
      try {
        ...
        // set default date range to last 7 days including today
  ...
  const stats = useMemo(() => {
    // Prefer backend daily analytics if available for visitors/averages, but compute totalSales from chart data
    if (backendStats) {
    ...
    // Prefer backend analytics rows if available
    if (reportRows && reportRows.length > 0) {
      ...
      const perf = acc; // use same as accuracy for now
      return {
        totalVisitors,
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      const normalize = (p: string) => {
        ...
        // Use last 11 digits to be robust with BD numbers (e.g., 8801XXXXXXXXX)
        return digits.length >= 11 ? digits.slice(-11) : digits;
      };

      // Fetch backend daily analytics for charts and cards
      const p = new URLSearchParams();
      p.set('from', start.toISOString().split('T')[0]);
      // Backend uses an exclusive upper bound ($lt end), so make end date inclusive by adding one day
      const endExclusive = new Date(end);
      endExclusive.setDate(endExclusive.getDate() + 1);
      p.set('to', endExclusive.toISOString().split('T')[0]);
      p.set('showroom', selectedShowroom);
      p.set('ts', String(Date.now()));
      const dailyRes = await fetch(`${baseUrl}/api/user/analytics/showroom-daily?${p.toString()}`, {
      ...
      // Also load aggregated analytics for stats/table
      const params = new URLSearchParams();
      params.set('from', start.toISOString().split('T')[0]);
      // Use the same inclusive end handling for aggregated report
      params.set('to', endExclusive.toISOString().split('T')[0]);
      if (selectedShowroom) params.set('showroom', selectedShowroom);
      params.set('ts', String(Date.now()));
      const repRes = await fetch(`${baseUrl}/api/user/analytics/showroom-report?` + params.toString(), {
        headers: { Authorization: `Bearer ${token}` },
        cache: 'no-store',
      });
      if (!repRes.ok) {
        // don't fail the whole generation; just clear rows
        setReportRows([]);
      } else {
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 mb-10">
            <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-8">
              ...
              <ResponsiveContainer width="100%" height={350}>
                <LineChart data={[...currentShowroomData.data].reverse()}>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-8">
              ...
              <ResponsiveContainer width="100%" height={350}>
                <BarChart data={[...currentShowroomData.data].reverse()}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" vertical={false} />
                  <XAxis dataKey="day" stroke="#94a3b8" />
                  <YAxis stroke="#94a3b8" />
                  <Tooltip
