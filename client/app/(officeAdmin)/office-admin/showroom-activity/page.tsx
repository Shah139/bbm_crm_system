...
// Mock data generator
const generateMockShowroomData = (): ShowroomActivity[] => {
  const showrooms = ['Showroom A', 'Showroom B', 'Showroom C', 'Showroom D', 'Showroom E'];
  const data: ShowroomActivity[] = [];

  for (let i = 0; i < 30; i++) {
    const showroomName = showrooms[Math.floor(Math.random() * showrooms.length)];
    const visitorCount = Math.floor(Math.random() * 300) + 50;
    
    // Generate customers for this showroom
    const customers: Customer[] = [];
    for (let j = 0; j < Math.min(visitorCount, 15); j++) {
      customers.push({
        id: j + 1,
        name: `Customer ${j + 1}`,
        email: `customer${j + 1}@email.com`,
        phone: `+1${Math.floor(Math.random() * 9000000000) + 1000000000}`,
        visitTime: `${String(Math.floor(Math.random() * 24)).padStart(2, '0')}:${String(Math.floor(Math.random() * 60)).padStart(2, '0')}`,
        purchaseAmount: Math.random() > 0.3 ? Math.floor(Math.random() * 500) + 50 : undefined,
      });
    }
...
export default function ShowroomActivityPage() {
  const [activityData] = useState<ShowroomActivity[]>(generateMockShowroomData());
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-8 hover:shadow-md transition-shadow duration-300">
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                <p className={`text-5xl font-bold ${getAccuracyColor(parseFloat(stats.avgAccuracy as string))}`}>
                  {stats.avgAccuracy}%
                </p>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
          ...
          <div className="p-8 border-b border-slate-100">
            <h2 className="text-xl font-bold text-slate-900">
              Activity Records ({filteredData.length})
            </h2>
          </div>
                ...
                {filteredData.length > 0 ? (
                  filteredData.map((activity, idx) => (
                    ...
                    >
                      ...
                      <td className="px-8 py-5 text-sm">
                        <span className={`px-3 py-1.5 rounded-lg text-xs font-bold ${getAccuracyColor(activity.accuracy)}`}>
                          {activity.accuracy}%
                        </span>
                      </td>
                      <td className="px-8 py-5 text-sm">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="font-semibold text-slate-900 text-xs min-w-[40px]">
                            {activity.performance}%
                          </span>
                        </div>
                      </td>
                      <td className="px-8 py-5 text-sm">
                        <span className={`px-3 py-1.5 rounded-lg text-xs font-bold ${getStatusColor(activity.status)}`}>
                          {activity.status}
                        </span>
                      </td>
                      <td className="px-8 py-5 text-sm">
                        <button
                          ...
                          className="inline-flex items-center gap-2 px-4 py-2 bg-blue-50 text-blue-700 rounded-lg hover:bg-blue-100 transition font-semibold text-xs border border-blue-200"
                        >
                          <Eye size={16} />
                          View Details
                        </button>
            ...
            <div className="p-8">
              ...
              <div className="grid grid-cols-3 gap-4 mb-8">
                ...
                <div className="bg-emerald-50 rounded-lg p-4 border border-emerald-200">
                  ...
                  <p className={`text-2xl font-bold ${getAccuracyColor(selectedActivity.accuracy)}`}>
